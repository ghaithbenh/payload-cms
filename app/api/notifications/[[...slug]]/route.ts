...
async function withAuthAndRateLimit(method: PayloadRouteHandler, request: Request, context: { params: Promise<{ slug?: string[] }> }) {
  ...
  const newParams = Promise.resolve({ ...resolvedParams, slug: ['notifications', ...slug] })
...
export async function GET(request: Request, context: { params: Promise<{ slug?: string[] }> }) {
  try {
    ...
    const data = await cacheAside(cacheKey, () => payload.find(queryOptions), CACHE_TTL.notifications)
